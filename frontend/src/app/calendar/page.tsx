...
interface Booking {
  ...
  booking_status: string; // confirmed, incomplete, waitlist, cancelled
  booked_at: string;
...
export default function MyCalendarPage() {
    ...
    const fetchBookings = async () => {
      try {
        ...
        setBookings(
          data.filter(
            (b) =>
              new Date(`${b.event.event_date}T${b.event.event_time}`) >= now
          )
        );
      } catch (err: any) {
  ...
  // Fill in leading empty days for alignment
